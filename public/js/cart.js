...
const Cart = {
  ...
  /** Shipping: PKR 300, free above PKR 3,000 (after discount) */
  SHIPPING_FEE: 300,
  FREE_SHIPPING_THRESHOLD: 3000,

  getShippingFee(subtotalAfterDiscount) {
    const amount = subtotalAfterDiscount != null ? subtotalAfterDiscount : this.getCartTotal();
    return amount >= this.FREE_SHIPPING_THRESHOLD ? 0 : this.SHIPPING_FEE;
  },

  getOrderTotals(discount = 0) {
    const subtotal = this.getCartTotal();
    const d = Math.min(subtotal, Math.max(0, discount || 0));
    const after = subtotal - d;
    const shipping = this.getShippingFee(after);
    return {
      subtotal,
      discount: d,
      ...
      amountToFreeShipping: Math.max(0, this.FREE_SHIPPING_THRESHOLD - after),
    };
  },
