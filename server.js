...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    const order = {
      ...
      subtotal: body.subtotal || items.reduce((s, it) => s + it.price * it.qty, 0),
      shippingFee: body.shippingFee != null ? body.shippingFee : 0,
      total: body.total || 0,
      notes: body.notes || '',
    };
    ...
    const addr = [order.shipping.address, order.shipping.apartment, order.shipping.city, order.shipping.postal, order.shipping.country].filter(Boolean).join(', ');
    sendMail({
      to: CONTACT_EMAIL,
...
Total: Rs ${order.total}`,
      html: `<h2>New order ${order.orderNumber}</h2>
        ...
        <p>Subtotal: Rs ${order.subtotal}<br>Shipping: Rs ${order.shippingFee}<br><strong>Total: Rs ${order.total}</strong></p>`,
    }).catch(console.error);
